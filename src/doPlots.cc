...
////  Color_t COLOR[] = {kOrange-3,kGreen-7,kMagenta-7,kAzure-3,kBlue,kViolet+2 ,kGreen-3, kRed-3};
  // testColor_t COLOR[] = {kAzure-3, kViolet-6, kTeal-5, kMagenta-3, kRed+1, kOrange+1, kYellow-4, kSpring-1}; 
  //Color_t COLOR[] = {kAzure-3, kViolet-9, kMagenta-9, kGreen+2, kRed+1, kOrange-4, kYellow-7, kAzure+6}; 
  Color_t COLOR[] = {kAzure-1, kRed, kCyan, kGreen+2, kOrange+10, kSpring-9, kCyan+0, kBlue+0};//{kRed-9, kViolet-9, kMagenta-10, kAzure+2, kCyan+2, kOrange-9, kYellow-9, kGreen-7};//kCyan-10, kYellow-9, kGreen-7};
  ...
  TTree *tree_redu[MAX_NUMBER];
  TTree *dataTree[MAX_NUMBER];
  TTree *bkgTree[MAX_NUMBER];
  TTree *signalTree[MAX_NUMBER];
  TTree *finalDataTree;
  TTree *finalBkgTree;
  TTree *finalSignalTree;
  ...
  int line_counter=0;
  finalDataTree = new TTree("dataTree","dataTree");
  finalSignalTree = new TTree("signalTree","signalTree");
  finalBkgTree = new TTree("bkgTree","bkgTree");
  ...
  while(true){
    ...
    tree[file_counter]->Draw(Form("%s>>histo_%d", variable.c_str(), file_counter),Form("(%s)",cut.c_str()));////photonCorrection*(%s)",cut.c_str()));
    //tree_redu[file_counter] = new TTree(Form("tree_redu_%d", file_counter),Form("tree_redu_%d", file_counter));
    //tree_redu[file_counter] = tree[file_counter]->CopyTree(Form("(%s)",cut.c_str())); 
    ...
    if(strcmp(bkg_nameTMP.c_str(), "data")==0){
      ...
      //dataTree[file_counter] = new TTree(Form("dataTree_%d", file_counter),Form("dataTree_%d", file_counter));
      //dataTree[file_counter] = tree[file_counter]->CopyTree(Form("%s"),cut.c_str());
      //dataList->Add(tree_redu[file_counter]);
      //finalDataTree->Add(dataTree[file_counter]);
      if(isData==0) {
      ...
      //std::string erasedName = bkg_nameTMP.erase(bkg_nameTMP.end()-bkg_nameTMP.length()+7);
      std::string erasedName = bkg_nameTMP;
      std::string s = "signal_";
///      signalTree[file_counter] = new TTree(Form("signalTree_%d", file_counter),Form("signalTree_%d", file_counter));
///      signalTree[file_counter] = tree[file_counter]->CopyTree(Form("%s"),cut.c_str());
      //signalList->Add(tree_redu[file_counter]);
      //finalSignalTree->Add(signalTree[file_counter]);

      std::string::size_type k = erasedName.find(s);

      if (k != std::string::npos){
           erasedName.erase(k, s.length());
      }

      signalHisto[sgn_counter]= new TH1D(Form("signalHisto_%d",atoi(erasedName.c_str())/*sgn_counter*/),Form("signalHisto_%d",atoi(erasedName.c_str())/*sgn_counter*/), bins,min,max);
      ...
      std::cout<<"bkg tree definition"<<std::endl;
      //bkgTree[file_counter] = new TTree(Form("bkgTree_%d", file_counter),Form("bkgTree_%d", file_counter));
      //std::cout<<"Reducing tree"<<std::endl;
      //bkgTree[file_counter] = tree[file_counter]->CopyTree(Form("%s"),cut.c_str());
      //bkgList->Add(tree_redu[file_counter]);
      //finalBkgTree->Add(bkgTree[file_counter]);
      if(doScale==0) {
        scaleFactor=1; 
        lumi=1; 
        counts=1;
      ...
      //  std::cout<<"BIN prima: "<<m<<" "<<histoD[file_counter]->GetBinContent(m)<<std::endl;
      //}
  ...
  if(isData==1){
    ...
    allBkgHisto->SetFillStyle(3018);
    allBkgHisto->SetFillColor(kBlack);

    dataHisto->Draw("EP");
    bkgStack->Draw("histoSAME");
    allBkgHisto->Draw("e2SAME");
...
////  for(int j=0; j<file_counter; ++j){
    //finalDataTree->MergeTrees(dataList);
    //finalDataTree->Write();
    //finalSignalTree->MergeTrees(signalList);
    //finalSignalTree->Write();
    //finalBkgTree->MergeTrees(bkgList);
    //finalBkgTree->Write();
////
////  }
