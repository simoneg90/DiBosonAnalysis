...
void fit_Fail()
{
  ...
  std::string cut_matched_pass = "((abs(lepton_pdgID)==11&&((abs(lepton_eta)>0&&abs(lepton_eta)<1.442)||(abs(lepton_eta)>1.56&&abs(lepton_eta)<2.5))&&metType1>80&&lepton_pt>120&&ak08Ungroomed_lepton_DR>1&&lepton_goodNumber==1)||(abs(lepton_pdgID)==13&&muonRelIso03<0.1&&metType1>40&&lepton_pt>40))&&btag>0&&ak08Ungroomed_1_pt>200&&WType1_pt>200&&ak08Ungroomed_1_tau21>0.6&&ak08Ungroomed_WGen_DR<0.1";
  ...
  std::string cut_pass = "((abs(lepton_pdgID)==11&&((abs(lepton_eta)>0&&abs(lepton_eta)<1.442)||(abs(lepton_eta)>1.56&&abs(lepton_eta)<2.5))&&metType1>80&&lepton_pt>120&&ak08Ungroomed_lepton_DR>1&&lepton_goodNumber==1)||(abs(lepton_pdgID)==13&&muonRelIso03<0.1&&metType1>40&&lepton_pt>40))&&btag>0&&ak08Ungroomed_1_pt>200&&WType1_pt>200&&ak08Ungroomed_1_tau21>0.6";
...
//  /*model_bkg*/p2.plotOn(frame1_bkg);
  ...
  RooRealVar mean_pass("mean_pass","mean_pass", 80, 78,105);
  ...
  RooRealVar a_pass("a_pass","a_pass", .1,-10,10);
  RooRealVar a1_pass("a1_pass","a1_pass", 0.1,-1,1);
  RooRealVar a2_pass("a2_pass","a2_pass", 0.1,-1,1);
  RooRealVar a3_pass("a3_pass","a3_pass", 0.1,-1,1);
  RooPolynomial p2_pass("p2_pass","p2_pass",ak08Pruned_1_mass_pass,RooArgList(a_pass,a1_pass,a2_pass),0) ;
  RooRealVar K("K","K", 0.7,0,1);
  RooExponential expPass("expPass", "expPass", ak08Pruned_1_mass_pass, a_pass);
  RooAddPdf testModel("testModel", "", RooArgList(expPass,p2_pass), RooArgList(K));

  RooChebychev cheby_pass("cheby_pass","cheby_pass",ak08Pruned_1_mass_pass,RooArgSet(a_pass,a1_pass, a2_pass)) ;
  ...
  RooAddPdf modelPass("modelPass", "modelPass", RooArgList(gx_pass,cheby_pass/*expPass,p2_pass*/), RooArgList( Nsig, Nbkg));
  ...
  //gx_pass.plotOn(frame1_pass, LineColor(kRed));//, Normalization(1.0,RooAbsReal::RelativeExpected));
  //expPass.plotOn(frame1_pass, LineColor(kGreen));
  //cheby_pass.plotOn(frame1_pass, LineColor(kGreen));//, Normalization(1.0,RooAbsReal::RelativeExpected));
//  p2_pass.plotOn(frame1_pass, LineColor(kGreen));
  modelPass.plotOn(frame1_pass);
  modelPass.paramOn(frame1_pass);
  double fitNSig=Nsig.getValV();
  double fitNBkg=Nbkg.getValV();
  ...
  int Ngen=1;
  mcstudy->generateAndFit(Ngen, 10000/*fitNSig+fitNBkg*/, kTRUE);
  ...
  RooPlot* frame6_MCStudy = mcstudy->plotPull(a_pass, Bins(40), FitGauss(kTrue));
  RooPlot* frame7_MCStudy = mcstudy->plotPull(a1_pass, Bins(40), FitGauss(kTrue));
  RooPlot* frame8_MCStudy = mcstudy->plotPull(a2_pass, Bins(40), FitGauss(kTrue));
  ...
  for(int i=0; i<Ngen;++i){
    ...
    pull->Fill((par1_fitresult->getValV()-(Nttbar_true.getValV()/*hh_ttbarMatch->GetEntries()*/+NStop_true.getValV()))/par1_fitresult->getError());
    std::cout<<"Pull: "<<(par1_fitresult->getValV()-(Nttbar_true.getValV()+NStop_true.getValV()))/par1_fitresult->getError()<<std::endl;
  }//end loop for pull
  mcstudy->genData(0)->plotOn(frame2_MCStudy);
  ...
  modelPass.fitTo((RooAbsData &)*(mcstudy->genData(0)));
  std::cout<<"After pdf!!!!"<<std::endl;
  modelPass.plotOn(frame2_MCStudy);
  modelPass.paramOn(frame2_MCStudy);
...
//  rfa1->Print();
  ...
  std::cout<<"NSig: "<<fitNSig<<" Nbkg: "<<fitNBkg<<std::endl;
